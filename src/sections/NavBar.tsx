    ...
    <nav className="bg-gradient-to-r from-slate-700 to-slate-900 sticky top-0 w-full backdrop-blur-lg z-50 border-b border-gray-950">
      <div className="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16 items-center">
          {/* Logo */}
          <div className="font-bold text-xl text-slate-950 bg-slate-50 py-2 px-4 rounded-full">Dukun.Coding</div>
          ...
          <div className="hidden lg:flex space-x-8">
            {navLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
                className="text-slate-50 hover:text-slate-400 transition-colors"
              >
                {link.name}
              </a>
            ))}
